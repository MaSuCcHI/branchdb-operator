...
func TestHandler_マルチプロバイダー構成でdb_type省略時はmysqlにフォールバックする(t *testing.T) {
	mysqlCalled := false
	...
	if w.Code != http.StatusOK {
		t.Errorf("got status %d, want 200 (db_type omitted should fall back to mysql)", w.Code)
	}
	if !mysqlCalled {
		t.Error("expected the mysql provider to be used as the deterministic default")
	}
}

func TestHandler_mysqlを含まないマルチプロバイダー構成でdb_type省略時に400を返す(t *testing.T) {
	provider1 := &mockVolumeProvider{
		listSnapshotsFunc: func(ctx context.Context) ([]domain.SnapshotInfo, error) {
			return nil, nil
		},
	...
	if w.Code != http.StatusBadRequest {
		t.Errorf("got status %d, want 400 when db_type is omitted and no mysql provider exists", w.Code)
	}
}
