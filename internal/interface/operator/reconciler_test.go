...
type mockDatabaseProvider struct {
	startFunc func(ctx context.Context, branchName string, volumeInfo domain.VolumeInfo, dbType, dbVersion string) (domain.BranchEndpoint, error)
...
func (m *mockDatabaseProvider) Stop(ctx context.Context, branchName string) error {
	if m.stopFunc != nil {
...
func newReconciler(scheme *runtime.Scheme, objs []runtime.Object, volumeProvider domain.VolumeProvider, dbProvider domain.BranchDatabaseProvider) *operator.DatabaseBranchReconciler {
...
func TestReconcile_VolumeProviderのCreateCloneが失敗したときphaseがErrorになる(t *testing.T) {
	...
	r := newReconciler(scheme, []runtime.Object{branch}, volumeProvider, &mockDatabaseProvider{})
...
func TestReconcile_削除フラグが立ったCRのクリーンアップが実行される(t *testing.T) {
	...
	r := newReconciler(scheme, []runtime.Object{branch}, volumeProvider, dbProvider)
...
func TestReconcile_存在しないCRを取得しようとしたときエラーなしで終了する(t *testing.T) {
	scheme := newScheme()
	r := newReconciler(scheme, []runtime.Object{}, &mockVolumeProvider{}, &mockDatabaseProvider{})
...
func newReconcilerWithInterceptor(scheme *runtime.Scheme, objs []runtime.Object, funcs interceptor.Funcs, volumeProvider domain.VolumeProvider, dbProvider domain.BranchDatabaseProvider) *operator.DatabaseBranchReconciler {
	fakeClient := fake.NewClientBuilder().
		WithScheme(scheme).
		WithRuntimeObjects(objs...).
		WithStatusSubresource(&v1alpha1.DatabaseBranch{}).
...
func TestReconcile_finalizerのUpdate失敗時にエラーを返す(t *testing.T) {
	...
	}, &mockVolumeProvider{}, &mockDatabaseProvider{})
