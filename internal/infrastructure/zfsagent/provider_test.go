...
func TestProvider_スナップショット作成に成功する(t *testing.T) {
	...
	err := provider.TakeSnapshot(context.Background(), "snap-001")
	if err != nil {
...
func TestProvider_クローン作成に成功しVolumeInfoを返す(t *testing.T) {
	...
	got, err := provider.CreateClone(context.Background(), "snap-001", "feature-login")
	if err != nil {
...
func TestProvider_クローン作成でサーバーがエラーを返したときエラーを返す(t *testing.T) {
	...
	_, err := provider.CreateClone(context.Background(), "snap-001", "feature-login")
	if err == nil {
		t.Error("expected error, got nil")
	}
}
...
func TestProvider_クローン削除に成功する(t *testing.T) {
	...
	err := provider.DeleteClone(context.Background(), "feature-login")
	if err != nil {
		t.Errorf("unexpected error: %v", err)
	}
}
...
func TestProvider_スナップショット一覧を取得する(t *testing.T) {
	...
	got, err := provider.ListSnapshots(context.Background())
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestProvider_トークンがAuthorizationヘッダーに含まれる(t *testing.T) {
	...
	_ = provider.TakeSnapshot(context.Background(), "snap-001")
...
func TestProvider_コンテキストキャンセル時にエラーを返す(t *testing.T) {
	...
	err := provider.TakeSnapshot(ctx, "snap-001")
	if err == nil {
...
func TestProvider_無効なURLのときCreateCloneはエラーを返す(t *testing.T) {
	provider := zfsagent.NewProvider("://invalid-url", "token")
	_, err := provider.CreateClone(context.Background(), "snap-001", "clone-1")
	if err == nil {
		t.Error("expected error for invalid URL, got nil")
	}
}
...
func TestProvider_無効なURLのときDeleteCloneはエラーを返す(t *testing.T) {
	provider := zfsagent.NewProvider("://invalid-url", "token")
	err := provider.DeleteClone(context.Background(), "clone-1")
	if err == nil {
		t.Error("expected error for invalid URL, got nil")
	}
}
...
func TestProvider_無効なURLのときListSnapshotsはエラーを返す(t *testing.T) {
	provider := zfsagent.NewProvider("://invalid-url", "token")
	_, err := provider.ListSnapshots(context.Background())
	if err == nil {
		t.Error("expected error for invalid URL, got nil")
	}
}
...
func TestProvider_クローン削除で404のときエラーなし(t *testing.T) {
	...
	if err := provider.DeleteClone(context.Background(), "feature-login"); err != nil {
